...
function transformData( pcarsdata ){
	var retData 		= {};
	retData['laps'] 	= [];		//init attribute
	retData['lapped']	= [];		//init, currently not know what value is here expected
	var cnt 			= 0;

	if(log >= 3){console.log('+++++++++++ transformData() input: ', pcarsdata );}
	
	var aDriversData = pcarsdata['drivers'];
	for ( var drivername in aDriversData ){		
		//copy data for each driver in new format	
<<<<<<< HEAD
		console.log('+++++++++++ transformData() push() ', aDriversData[drivername] );
		
		// filter out demo data
		if ( drivername == 'Slightly Mad Studios Ltd'){
			continue;
		}
		
=======
		if(log >= 3){console.log('+++++++++++ transformData() push() ', aDriversData[drivername] );}
>>>>>>> 08c31d80ea46674eb31d034645fb502b5f00c117
		retData['laps'].push( aDriversData[drivername] );		
		cnt++;
	}
	
	//
	for (var i = 0;    i < pcarsdata['lapCount'];    i++){		
		retData['lapped'].push( "-1" );	
	}
		
				
	retData['lapCount'] = pcarsdata['lapCount'];	
	retData['safety']	= [];
	
	if(log >= 3){console.log('+++++++++++ transformData() output: ', retData );}
	
	return retData;
	
}

function removeOldStatistics( htmlID ){
	
	// remove old versions of a chart in case of multiple calls
	d3.select("#chart").selectAll("*").remove();
	
	return true;
}

function initLap_LapChart ( data ) {
    // Load data.
//d3.json("/pcars/lib/LapChart/Christoph_2010au.json", function(data) {	
	
		removeOldStatistics("#chart");
	
	if(log >= 3){console.log('+++++++++++ LapChart data input: ', data );}
    	
    	if (data['lapCount'] >= 10000 || data['lapCount'] <= 0	){    		
    		if(log >= 3){console.log("Number of lapcount unrealistic");}
    	}
...
function checkLaps(laps, lapCount) {
    ...
    for (var j = 0;
        ...
        if (name == undefined || name.length == 0) {
            ...
            if(log >= 3){console.log("Warning: invalid name for element " + j);}
        }
        ...
        if (places == undefined) {
            ...
            if(log >= 3){console.log("Warning: missing placings for element " + j + " (" + name + ")");}
        }
        else if (places.length == 0 || places.length > lapCount + 1) {

            if(log >= 3){console.log("Warning: invalid number of placings (" + places.length + ") for element " + j +
    ...
    for (var i = 0;
        ...
        for (j = 0;
            ...
            if (places.length > i) {
                ...
                if (isNaN(placing) || placing < 1 || placing % 1 != 0) {
                    ...
                    if(log >= 3){console.log("Warning: invalid placing '" + placing + "' for " + laps[j].name)}
                }
                else {
        ...
        for (j = 1;
            ...
            if (count != 1) {
                ...
                if(log >= 3){console.log("Warning: data inconsistent: lap " + i + ", position " + j + ", count " + count);}
            }
        }
    }
}
    ...
    if (marker != undefined) {
        ...
        for (var i = 0;
            ...
            if (isNaN(stop) || stop < 0 || stop >= max || stop % 1 != 0) {
                ...
                if(log >= 3){console.log("Warning: invalid " + type + " (" + stop + ") for element " + index + " (" + name + ")");}
            }
        }
    }
}
...
function checkLapped(lapped, lapCount, driverCount) {
    ...
    if (lapped != undefined) {
        ...
        if (lappedLength != lapCount) {
            ...
            if(log >= 3){console.log("Lapped array length (" + lappedLength + ") incorrect - expected length " + lapCount);}
        }
        ...
        for (var j = 1;
            ...
            if (isNaN(position) || position % 1 != 0 || position < -1 || position > driverCount) {
                ...
                if(log >= 3){console.log("Invalid lapped position: element " + j + " (" + position
    ...
    if (safety != undefined) {
        ...
        for (var i = 0;
            ...
            if (isNaN(lap) || lap < 0 || lap % 1 != 0 || lap > lapCount) {
                ...
                if(log >= 3){console.log("Invalid safety car lap: element " + i + " (" + lap
